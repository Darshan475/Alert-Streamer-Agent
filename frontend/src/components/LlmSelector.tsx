...
export function LlmSelector({ onChanged }: Props) {
  ...
  const statusText = (() => {
    ...
    if (active.id === "offline") return "Offline fallback · no API key needed";
    if (activeConfigured) return `${active.label} · ${active.model}`;
    return `No API key — ${active.key_hint}`;
  })();
